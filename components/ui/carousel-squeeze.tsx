...
export type SqueezeSlide = {
    ...
    description?: string;
    /** Subtitle or category line */
    subtitle?: string;
    /** Extended bullet points or detailed description */
    longDescription?: string;
    /** Accent color class for the subtitle/accents */
    ...
    onAction?: () => void;
    /** Custom extra content for the panel */
...
export function SqueezeCarousel({
    ...
    useEffect(() => {
        if (activeIndex !== undefined && activeIndex >= 0 && activeIndex < count && activeIndex !== open) {
            go(activeIndex);
        }
    }, [activeIndex, open, count, go]);
    ...
    const vars = {
        ...
        "--sq-fill": (slides[open] as any)?.accent || accent,
    ...
    return (
        ...
        >
            ...
            <div id={`${ids}-panel`} role="tabpanel" aria-live="polite" className="mt-8 grid">
                {slides.map((slide, i) => {
                    const shown = i === open;
                    ...
                    return (
                        ...
                        >
                            {slide.subtitle || slide.longDescription || slide.customContent ? (
                                <div className="max-w-[48rem] flex flex-col gap-3">
                                    {slide.subtitle && (
                                        <span className={cn("text-xs sm:text-sm font-semibold tracking-wide uppercase", slide.accentColorClass)}>
                                            {slide.subtitle}
                                        </span>
                                    )}
                                    ...
                                    {slide.longDescription && (
                                        <div className="mt-2 text-slate-600 dark:text-slate-300 text-sm sm:text-base whitespace-pre-line border-t border-slate-200/60 dark:border-slate-800 pt-4 leading-relaxed font-normal">
                                            {slide.longDescription}
                                        </div>
                                    )}
                                    {slide.customContent}
